...
import com.sunday.android.yun.yunweather.utils.SpannalbeStringUtils;
...
public class IndexAdapter extends RecyclerView.Adapter implements IViewHelper {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position == 0) {
            ...
            String str2 = String.format("相对湿度\n%s", nowBean.getHum());
            String str3 = String.format("体感温度\n%s°", nowBean.getFl());
            headHolder.tvNowWind.setText(SpannalbeStringUtils.setTextSizeDp(str1, 18, nowBean.getWind().getDir().length()));
            headHolder.tvNowHumidity.setText(SpannalbeStringUtils.setTextSizeDp(str2, 18, 4));
            headHolder.tvNowFeel.setText(SpannalbeStringUtils.setTextSizeDp(str3, 18, 4));
        } else if (position == 1) {
            ...
            for (Weather.HeWeather5Bean.DailyForecastBean bean : dailyForecastBeanList) {
                ...
                getTextView(constraintLayout, R.id.tv_day).setText(days[i]);
                getTextView(constraintLayout, R.id.tv_simple_info).setText(String.format("%s", bean.getCond().getTxt_d()));
                getTextView(constraintLayout, R.id.tv_temp).setText(String.format("%s° / %s°", bean.getTmp().getMax(), bean.getTmp().getMin()));
                Picasso.with(mContext)
                        .load(String.format(ApiClient.ICON_URL + "%s.png", bean.getCond().getCode_d()))
                        .centerInside().resize(100, 100).placeholder(R.drawable.ic_unknow)
                        .into(getImageView(constraintLayout, R.id.imageView));
            ...
            contentHolder.tvTitle.setText("每小时预报");
            ...
            for (Weather.HeWeather5Bean.HourlyForecastBean bean : hourlyForecastBeanList) {
                ...
                Picasso.with(mContext)
                        .load(String.format(ApiClient.ICON_URL + "%s.png", bean.getCond().getCode()))
                        .centerInside().resize(100, 100).placeholder(R.drawable.ic_unknow)
                        .into(getImageView(constraintLayout, R.id.imageView));
                getTextView(constraintLayout, R.id.tv_day).setText(bean.getDate().substring(bean.getDate().length() - 6, bean.getDate().length()));
                getTextView(constraintLayout, R.id.tv_simple_info).setText(String.format("%s | %s", bean.getCond().getTxt(), bean.getWind().getSc()));
            ...
            for (int i = 0; i < 4; i++) {
                ...
                ImageView imageView = getImageView(constraintLayout, R.id.imageView);
                TextView day = getTextView(constraintLayout, R.id.tv_day);
                TextView simpleInfo = getTextView(constraintLayout, R.id.tv_simple_info);
                switch (i) {
                    case 0:
